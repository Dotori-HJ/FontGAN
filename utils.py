...
def read_split_image(img):
    mat = imageio.imread(img).astype(np.float32)
...
def tight_crop_image(img, verbose=False, resize_fix=False):
    ...
    col_sum = np.where(np.sum(img < 200, axis=0) > 0)
    row_sum = np.where(np.sum(img < 200, axis=1) > 0)
    ...
    if type(resize_fix) == int:
        ...
        cropped_image = cv2.resize(cropped_image, (resize_w, resize_h))
        cropped_image_size = cropped_image.shape
        if verbose:
...
def add_padding(img, image_size=128, verbose=False, pad_value=0):
